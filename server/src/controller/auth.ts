...
export const updateUser =async(req: express.Request,res: express.Response)=>{
 ...
 const updatedUser =await User.findOneAndUpdate({_id:req.user.userId},{
...
export const login =async(req:express.Request,res:express.Response)=>{
 ...
 const passwordMatch = await bcrypt.compare( password,user.password);
   if(passwordMatch){
const token = jwt.sign({userId:user._id,email:user.email},"your_secret_key",{expiresIn:"30d"});
res.send({token})
    }else{
...
export  const getAuthenticatedUser =async(req: express.Request,res: express.Response) =>{
 if(req.user){
